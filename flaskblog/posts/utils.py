...
from flaskblog.models import PostHistory, DeletedPost, DeletedPostHistory
...
# def create_mass():
    ...
    # with open(r'flaskblog\my_functions\posts.json') as f:
    # posts = json.load(f)
    # posts =

    # posts = json.loads(posts)
    # for pst in posts:
    #     print(str(pst["title"]) + " " + str(pst["content"]) + " "
    #           + str(pst["user_id"]))
    #     post = Post()
    #     post = Post(title=pst['title'], content=pst['content'],
    #                 user_id=int(pst['user_id']))
    #     db.session.add(post)
    #     db.session.commit()
